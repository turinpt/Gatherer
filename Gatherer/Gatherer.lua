...
--Gatherer_Print("DEBUG Current node: "..Gatherer_currentNode);
	...
	elseif (event == "WORLD_MAP_UPDATE") then
		if (WorldMapFrame:IsVisible()) then
			...
			if (GetRealZoneText() == "Alterac Valley") then 
				--mapContinent = 3; 
				--mapZone = 1;
			end
			if (GatherConfig.mapMinder == true and Gatherer_MapOpen == false) then 
				...
				if ( mapContinent>0 and startZone and startZone > 0 ) then
		...
		if (ClosestGathers and ClosestGathers.count > 0) then
			...
			for closestPos, closestGather in ClosestGathers.items do
				...
				if ( skip_node == 0 ) then
					...
					if (iconSet == "shaded" and closestGather.item.lastpick) then
						ShowTimer(closestGather.item.lastpick,gatherNote,textureIcon)
					else
						HideTimer(gatherNote)
					end
...
--							getglobal("GatherMain"..lastUnused);
							...
							if (gatherInfo.lastpick) then
								ShowTimer(gatherInfo.lastpick, mainNote, specificType)
							else
								HideTimer(mainNote)
							end
...
function ShowTimer(lastPick, parent, nodeName)
	...
	if (not timer) then
		...
		timer:SetWidth(40)
		timer:SetHeight(20)
		timer:SetPoint("LEFT", parent, "RIGHT", 3, 0)

		text = timer:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
		text:SetPoint("LEFT", timer, "LEFT", 0, 0)
		text:SetTextColor(1, 1, 1, 1)
		timer.text = text;
	end

	local respawnTime = lastPick + GetNodeTime(nodeName);
	local timeLeft = respawnTime - time();
	if (timeLeft > 300) then
		timer.text:SetText(ReturnNewDuration(timeLeft))
		timer.text:SetTextColor(1, 1, 1, 1)
	elseif (timeLeft > 0) then
		timer.text:SetText(ReturnNewDuration(timeLeft))
		timer.text:SetTextColor(1, 1, 0, 1)
	elseif (timeLeft > -300) then
		timer.text:SetText(ReturnNewDuration(timeLeft*-1))
		timer.text:SetTextColor(1, 0, 0, 1)
	elseif (timeLeft > -600 and nodeName == "chest") then
		timer.text:SetText(ReturnNewDuration(timeLeft*-1))
		timer.text:SetTextColor(1, 0, 0, 1)
	else
		timer:Hide();
...
function ReturnNewDuration(time)
...
function Gatherer_AddGatherToBase(gather, gatherType, gatherC, gatherZ, gatherX, gatherY, gatherIcon, gatherEventType, time)
		...
		if (gatherType == 0 or gatherType == 1) then
			GatherItems[gatherC][gatherZ][gather][found].lastpick = time;
		end
	end
...
function Gatherer_AddGatherHere(gather, gatherType, gatherIcon, gatherEventType)
	...
	local time = time();
	Gatherer_BroadcastGather(gather, gatherType, gatherC, gatherZ, gatherX, gatherY, gatherIcon, gatherEventType, 1);
...
function Gatherer_LoadZoneData()
	...
	--GatherZoneData["Alterac Valley"] = { [1] = 3, [2] = 1 };
end
...
function Gatherer_ChangeMap()
	...
	if ( mapContinent>0 and playerZone and playerZone > 0 ) then 
